...
const Odontograma = () => {
  ...
  const {getTeethFn, odontogramaEditable, loadingTeethList, setVisibleToothOptionsModalFn, setSelectedToothFn} = useContext(odontogramaContext);
  ...
  return (
    ...
    <ToothOption/>
    <div className={`${ondontoCss["title"]} ${ondontoCss["title--upper"]}`}>{patient?.name} {patient?.last_name}</div>
      <div className={ondontoCss["content__odont"]}>
        {(patient)?
          <Fragment>
          {(!loadingTeethList)?
            <Fragment>
        ...
        :
          <h4>No hay paciente seleccionado</h4>
        }
      </div>
      </div>
  );
